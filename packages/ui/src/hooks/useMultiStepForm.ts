...
export const useMultiStepForm = (
  ...
  const currentStepConfig: StepConfig =
    config.steps[currentStep] || config.steps[0]!;
  ...
  const stepErrors = useMemo(() => {
    ...
    config.steps.forEach((step) => {
      ...
      const hasErrors = stepFieldNames.some(
        (fieldName) => formMethods.errors[fieldName],
      );
  ...
  const canGoToStep = useCallback(
    (step: number): boolean => {
      ...
      return (
        step <= Math.max(...Array.from(completedSteps)) + 1 &&
        step >= 0 &&
  ...
  const nextStep = useCallback(() => {
    ...
    const shouldValidate =
      currentStepConfig?.validation === "onNext" ||
    ...
    const currentStepFieldNames =
      currentStepConfig?.fields?.map((field) => field.name) || [];
