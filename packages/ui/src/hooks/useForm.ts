...
import { FormConfig, FormDataType, FormErrors, UseFormReturn } from '@repo/ui/types';
...
export const useForm = (
  ...
  const [data, setData] = useState<FormDataType>(() => {
    ...
    config.fields.forEach(field => {
      if (initialData && field.name in initialData) {
        initialFormData[field.name] = initialData[field.name] as string | number | boolean | File | null;
      } else {
        switch (field.type) {
          case 'checkbox':
  ...
  const validateField = useCallback((name: string, value: string | number | boolean | File | null): string | null => {
