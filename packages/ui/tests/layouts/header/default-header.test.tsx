...
const mockHeaderData: HeaderDataType = {
  ...
  buttons: [
    { label: 'Sign In', href: '/signin', variant: 'outline' as const, size: 'sm' as const },
    { label: 'Sign Up', href: '/signup', variant: 'default' as const, size: 'sm' as const }
  ],
  mobileContent: { label: 'Get Started', href: '/get-started', variant: 'default' as const, size: 'sm' as const }
}
...
describe('Header', () => {
  ...
  it('logo is wrapped in a link to "/"', () => {
    render(<Header data={mockHeaderData} />)
    expect(screen.getByRole('link', { name: 'Company Logo' })).toHaveAttribute('href', '/')
  })
  ...
  describe('Desktop nav & buttons', () => {
    ...
    it('renders desktop buttons and they have the right attrs', () => {
      ...
      // Sign In link & button
      const signInLink = within(container).getByRole('link', { name: 'Sign In' })
      expect(signInLink).toHaveAttribute('href', '/signin')

      const buttons = within(container).getAllByRole('button', { name: 'Sign In' })
...
it('renders mobile links & mobile Sign In inside panel', () => {
      ...
      const mobileSignIns = within(panel).getAllByRole('button', { name: 'Sign In' })
  ...
  describe('Click handlers', () => {
    it('fires desktop onClick when provided', () => {
      ...
      const data: HeaderDataType = {
        ...mockHeaderData,
        buttons: [{ label: 'Click Me', onClick, variant: 'default' as const, size: 'sm' as const }]
      }
      ...
      const clickButtons = within(container).getAllByRole('button', { name: 'Click Me' })
    ...
    it('fires mobile onClick when provided', () => {
      ...
      const data: HeaderDataType = {
        ...mockHeaderData,
        buttons: [{ label: 'Mobile Click', onClick, variant: 'default' as const, size: 'sm' as const }]
      }
      render(<Header data={data} />)
      ...
      const mobileButtons = within(panel).getAllByRole('button', { name: 'Mobile Click' })
